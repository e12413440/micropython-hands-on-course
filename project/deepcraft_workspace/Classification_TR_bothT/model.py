...
# 
# Generated at 02/17/2026 16:22:20 UTC. Any changes will be lost.
# 
...
_K5 = np.empty((6608), dtype=np.int8)	# byte (8 bit) 
...
class Model:
    ...
    def dequeue(self, data_out : np.array, time_out : np.array):
        ...
        Parameters:
         data_out(float[1200]): DESCRIPTION. [200,6]
         time_out(float[2]): DESCRIPTION. [2]
        """
        
        return_status = self._K5.dequeue(data_out.reshape([200,6]), 40, time_out.reshape([2]))
        if return_status != ReturnStatus.RET_SUCCESS:
            return return_status
