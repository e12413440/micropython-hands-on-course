...
# 
# Generated at 02/16/2026 17:44:47 UTC. Any changes will be lost.
# 
# Layer                          Shape           Type       Function
# Sliding Window (data points)   [400,6]         float      dequeue
# Contextual Window (Sliding Window) [400,6]         float      dequeue
# 

import numpy as np
import enum

_K5 = np.empty((13008), dtype=np.int8)	# byte (8 bit) 
...
class Model:
    ...
    def dequeue(self, data_out : np.array, time_out : np.array):
        ...
        Parameters:
         data_out(float[2400]): DESCRIPTION. [400,6]
         time_out(float[2]): DESCRIPTION. [2]
        """
        
        return_status = self._K5.dequeue(data_out.reshape([400,6]), 40, time_out.reshape([2]))
        if return_status != ReturnStatus.RET_SUCCESS:
            return return_status
